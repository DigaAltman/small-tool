    ...
    private String html(int level) {
        ...
        if (node.element.elements().size() > 0) {
            ...
            for (Element element : node.element.elements()) {
                XmlNode xmlNode = new XmlNode(UUID.randomUUID().toString(), element, level + 1);
                String innerHtml = new XmlQuery(xmlNode.element, xmlNode.level).html(level + 2);
                res.append("\n").append(innerHtml);
            }
        } else {
            noWrap = true;
        ...
        return res.append("</").append(tagName()).append(">").toString();
    }
