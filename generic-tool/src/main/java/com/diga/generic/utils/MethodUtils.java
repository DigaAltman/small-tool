            ...
            synchronized (methodId) {
                ...
                // 如果它是一个接口 或者 它是一个抽象类. 那么我们在进行 JSON 转换的时候是会抛出异常的. 为了解决这个问题. 我们需要拿到它的实现类
                    ...
                    if (implementationClass.size() > 1) {
                        throw new RuntimeException("返回值类型为 [" + returnType.getName() + "] 的方法" + method.getName() + " 无法完成序列化操作, 因为它的实现类太多了. 如果要反序列化, 必须保证返回值类型为具体对象, 或者是只有一个子类的抽象类和接口");
                    }
                    if (implementationClass.size() == 0) {
                        throw new RuntimeException("返回值类型为 [" + returnType.getName() + "] 的方法 " + method.getName() + " 无法完成序列化操作, 因为返回类型在目前我们定义的类中找不到实现");
                    }
